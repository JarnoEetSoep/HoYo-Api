...
impl Client {
    ...
    pub fn destructure_cookie(
        cookie: &str,
    ) -> Result<(String, String, String, String, String, String), CookieError> {
        lazy_static! {
    ...
    pub async fn claim_daily(&self) -> Result<(), Box<dyn Error>> {
        ...
        let url =
            Url::parse_with_params(self.uid.get_region().get_claim_daily_url(), params).unwrap();
        ...
        if body["retcode"] == -5003 {
            // Already claimed
            return Err(Box::from(HoyoError(
                "Nay 't was already claimed today!".to_string(),
            )));
        }
    ...
    pub async fn claim_code(&self, code: &str) -> Result<(), Box<dyn Error>> {
        ...
        if body["retcode"] == -2003 {
            ...
            return Err(Box::from(HoyoError(
                "Code redemption in cooldown".to_string(),
            )));
